...
export default function PantsPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter only pants added by admin
        const pants = allProducts.filter((p: Product) => p.category === "pants");
        setProducts(pants);
      } catch (error) {
        console.error("Failed to fetch pants:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const addToCart = (product: any) => {
    ...
    if (!user) {
      ...
      router.push("/login?redirect=/pants");
      return;
    }
  ...
  return (
    <div className="page">
      ...
      {loading ? (
        ...
        <p style={{ textAlign: "center" }}>No pants available. Admin will add products soon.</p>
      ) : (
